...
data class RESpeckLiveData(val phoneTimestamp: Long, val respeckTimestamp: Long,
                           ...
                           val breathingSignal: Float, val breathingRate: Float, val isBreathStart: Boolean,
                           val activityLevel: Float, val activityType: Int, val avgBreathingRate: Float,
    ...
    fun toStringForFile(): String {
        return phoneTimestamp.toString() + "," + respeckTimestamp + "," + sequenceNumberInBatch + "," + accelX +
                ...
                "," + activityType + "," + if (isBreathStart) 1 else 0
    }
}